...
//Power On sequence //
         ...
         LCD_WR_REG(0x10, 0x1190); // SAP, BT[3:0], AP, DSTB, SLP, STB
         LCD_WR_REG(0x11, 0x0227); // DC1[2:0], DC0[2:0], VC[2:0]
         delay_ms(1000); // Delay 50ms
        
         LCD_WR_REG(0x12, 0x001a); // Internal reference voltage= Vci;
         delay_ms(1000); // Delay 50ms
         LCD_WR_REG(0x13, 0x1100); // Set VDV[4:0] for VCOM amplitude
         LCD_WR_REG(0x29, 0x000e); // Set VCM[5:0] for VCOMH
         LCD_WR_REG(0x2B, 0x000c); // Set Frame Rate
         delay_ms(400); // Delay 50ms
...
// ----------- Adjust the Gamma Curve ----------//
		...
		LCD_WR_REG(0x0036,0x0202);
		LCD_WR_REG(0x0037,0x0000);
		LCD_WR_REG(0x0038,0x0202);
		LCD_WR_REG(0x0039,0x0505);
		LCD_WR_REG(0x003C,0x0202);
		LCD_WR_REG(0x003D,0x1f01);
//------------------ Set GRAM area ---------------//
